...
const Dashboard = () => {
  ...
  const { data, isLoading } = useAIAnalysis(user?._id); 
  ...
  {/* ✅ Wrap Loading/Wallet Section and AI Insights in a Flex Container */}
  <div className="flex flex-col md:flex-row gap-6 w-full max-w-4xl mt-6 overflow-auto">
    {/* 🏦 Wallet Section (Left Side) */}
    <div className="flex-1 bg-white p-6 rounded-lg shadow-md text-center">
      {loading ? (
        ...
        <p className="text-gray-500">Waiting for wallet creation...(Refresh if already created)</p>
      )}
    </div>

    {/* 🤖 AI Financial Insights (Right Side) */}
    <div className="flex-1 bg-white p-6 rounded-lg shadow-md overflow-auto">
  ...
  {showModal && (
    <div className="overflow-auto flex items-center justify-center bg-black bg-opacity-50">
